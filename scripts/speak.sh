...
# Env:
#   SPEAK_VOICE  macOS voice name (default: "Linh (Premium)" — falls back to Linh if not installed)
#   SPEAK_RATE   words per minute (default: 265 — ~1.5x Linh's natural pace)
set -euo pipefail

TEXT="${1:?Usage: speak.sh \"text\"}"
VOICE="${SPEAK_VOICE:-Linh (Premium)}"
RATE="${SPEAK_RATE:-265}"

# Graceful fallback: if requested voice isn't installed, drop to plain Linh.
# `say -v "?"` lines look like: `Linh (Premium)      vi_VN    # Xin chào!...`
...
if ! say -v "?" | sed -E 's/[[:space:]]+[a-z]+_[A-Z]+[[:space:]]+#.*$//' | grep -qxF "$VOICE"; then
  ...
  echo "         Cài Premium: System Settings → Accessibility → Spoken Content → Manage Voices → Vietnamese → Linh → Premium" >&2
  VOICE="Linh"
fi
